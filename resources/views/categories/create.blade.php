    ...
    <div class="max-w-2xl mx-auto rounded-2xl bg-white p-6 sm:p-8 shadow-sm border border-slate-200">
        ...
        <div class="mb-8 flex items-center justify-between gap-4 border-b border-slate-150 pb-5">
            <div>
                ...
                <p class="text-sm text-slate-500">Define a new drug class or classification segment.</p>
            </div>
        ...
        <form action="{{ route('categories.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name') }}" placeholder="e.g. Antibiotics" class="w-full rounded-lg border border-slate-200 bg-slate-50/50 px-3.5 py-2.5 text-sm text-slate-900 focus:border-teal-500 focus:bg-white focus:outline-none transition-all duration-150 shadow-xs" required>
            </div>
